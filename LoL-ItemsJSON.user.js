...
//
const reduceChampions = (champions) =>
  Object.entries(champions).reduce(
    (acc, [champ, value]) => ({
      ...acc,
...
//
const reduceNonSameChamps = (champions) =>
  Object.entries(champions)
    .filter(([champ, value]) => champ !== value.name)
    .reduce(
      (acc, [champ, value]) => ({
        ...acc,
        [value.name.replaceAll(" ", "").replace("'", "").toLowerCase()]:
          champ.toLowerCase(),
      }),
      {}
    );
...
const RiotAPI = function () {
  ...
  self.currentPatch = async () => {
    try {
      const version = await self.request.get(
  ...
  const _getLangs = async () => {
    ...
    try {
      const langs = await self.request.get(
        "https://ddragon.leagueoflegends.com/cdn/languages.json"
      );
  ...
  self.getResources = async (version, resource) => {
    if (["item", "champion"].includes(resource) === false) {
      throw new Error(
        'Invalid resource type. Only "items" and "champions" are allowed'
      );
    }
    try {
      if (resource === "item") {
          ...
          langs.map(
            async (lang) =>
              await self.request.get(
                `https://ddragon.leagueoflegends.com/cdn/${version}/data/${lang}/${resource}.json`
              )
          )
        );
        const finalItems = items.reduce(
          (acc, item) => ({ ...acc, ...reduceItems(JSON.parse(item).data) }),
          []
        );
        if (finalItems.muramana && finalItems.manamune) {
          finalItems.muramana = finalItems.manamune;
        }
        return finalItems;
      }
      const champsJson = await self.request.get(
        `https://ddragon.leagueoflegends.com/cdn/${version}/data/en_US/${resource}.json`
      );
...
const ItemSet = function (data, itemCodes, championCodes, needToAddSpaces) {
  ...
  const getChampCode = () =>
    championCodes[data.champion] ||
    championCodes[needToAddSpaces[data.champion]];
  // Make Blocks
  const itemBlocks = {
    associatedMaps: [],
    title: data.title + " - " + data.author,
    associatedChampions: data.champCode
      ? [parseInt(data.champCode, 10)]
      : [getChampCode(data.champion)],
...
const LolItemsJson = function ({ logs = true } = {}) {
  ...
  let _needToAddSpaces =
    localStorage.getItem(`${_prefix}needToAddSpaces`) || null;
  ...
  const _init = async () => {
    try {
      ...
      const response = await riotAPI.getResources(
        _installedVersion,
        "champion"
      );
      ...
      localStorage.setItem(
        `${_prefix}championCodes`,
        JSON.stringify(_championCodes)
      );
      localStorage.setItem(
        `${_prefix}needToAddSpaces`,
        JSON.stringify(_needToAddSpaces)
      );
      log("*** Champs Loaded... ***");
      //
    } catch (error) {
      alert(
        "THERE WAS AN ERROR WHILE INITIALIZING THE SCRIPT. PLEASE RELOAD THE PAGE AND TRY AGAIN. IF THE ERROR PERSISTS, PLEASE CONTACT THE DEVELOPER."
      );
    }
  };
  self.register = async () => {
  ...
  const _scrapeFromMobaFire = () => ({
    title: document
      .querySelector(".view-guide__banner__title span")
      .innerText.trim(),
    author:
      document.querySelector(".view-guide__banner__author").querySelector("a")
        .innerHTML +
      " @ " +
      document.querySelector(".view-guide__banner__author").querySelector("a")
        .href,
    champion: document
      .querySelector(".champ-tabs__more")
    ...
    items: [
      ...document.querySelectorAll(
        ".view-guide__build__items .view-guide__items"
      ),
    ].map((item) => {
  ...
  const _scrapeFromProBuilds = () => ({
    ...
    champCode: document
      .querySelector("td.summoner.highlighted")
      .parentElement.querySelector(".champion-icon .champ-img").dataset.id,
    items: [
      {
        title: document.querySelector(".guide-items div.left:first-of-type h3")
          .innerText,
        content: [
          ...document.querySelectorAll(
            ".guide-items div.left:first-of-type ul li"
          ),
        ]
          .map((it) => it.querySelector("p").innerText.toLowerCase())
          .filter((i) => i),
      },
      ...[
        ...document.querySelectorAll(
          ".guide-items div.buy-order ul.buy-order li"
        ),
      ].reduce((acc, it, index) => {
  ...
  const _makeButton = () => {
    ...
    if (location.includes("mobafire.com")) {
      const parent = document.querySelector(
        ".view-guide__build__items .collapse-title"
      ).parentElement;
      parent.insertBefore(
        buttonWrapper,
        parent.querySelector(".view-guide__build__items .collapseBox")
      );
    }
